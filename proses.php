...
if (isset($_POST['save2'])) {
    ...
    if ($size < 2048000 and ($type == 'image/jpeg' or $type == 'image/png')) {
        move_uploaded_file($_FILES['foto_produk']['tmp_name'], $folder . $filename);
        mysqli_query($koneksi, "insert into produk2(size,type,foto_produk,nama_produk,harga_produk,stok_produk,desc_produk,link_produk,wa_produk,kategori_produk) values('$type','$size','$filename','$nama_produk','$harga_produk','$stok_produk','$desc_produk','$link_produk','$wa_produk','$kategori_produk')");
        header('location:index.php');
    } else {
        echo "<b>Gagal Upload File</b>";
    }
